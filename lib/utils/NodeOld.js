...
{
	this.Page = AVObj.extend( { initStaticObject: true, preventExtensions: true } );
	this.Node = AVObj.extend( { preventExtensions: false } );
	this.node = {};
	this.text = {};
	this.P = {};
});*/
...
{
	/**
	 * @class AV.Utilities.Page
	 * @staticclass
	 * @classdesc The Page Class handles {@link node} creation and management.
	 * @extends AVObj
	 */
	$scope.Page = AVObj.extend( { initStaticObject: true, preventExtensions: true } ).prop(
		{
			/**
			...
			find: function ( nodeName, parent )
			{
				...
				return $scope.Page.generateNodeFromElements( nodeFound );
			},
			...
			generateNodeFromElements: function ( nodeObjects )
			{
				...
				return ( !nodeObjects._nodeDat ? Node.create( nodeObjects ) : nodeObjects._nodeDat );
			},
			...
			build: function ( nodeName, nodeProperties )
			{
				...
				for ( var i in nodeProperties )
				{
					...
					{
					case "text":
						n.appendChild( text( nodeProperties[i] ) );			// These two commands should be changed later to something more dom compliant.
						break;
					case "htmlText":
	...
	$scope.Node = AVObj.extend( { preventExtensions: false } ).prop(
	{
		events: {
			/**
		...
		text: function ( string, isHtml )
		{
			if ( !isHtml )
			{
				this.element.appendChild( Page.text( string ) );
			}
			else
			{
		...
		//TODO fix up observer calls as they are completely broken.
		//TODO to fix, I will have to go in and change the way objects are stored and removed as neither is working correctly.
		/**
		...
		{
			if ( !this.bindedEvents[eventName] )
			{
				this.bindedEvents[eventName] = {};
			}

			// Create the initial event if it does not exist or the one bounded was a different event.
			if ( !this.bindedEvents[eventName][functionName] )
			{
				var bindedObject = this.bindedEvents[eventName][functionName] = this.fireEvent.bind( this, eventName, data );
				this.element.addEventListener( eventName, bindedObject, false );
			}
			this.addObserver( eventName, functionName, scope );
		},

		addANObserver : function( eventName, refName, method, scope, data )
		{
			if ( !this.bindedEvents[eventName] )
			{
				this.bindedEvents[eventName] = {};
			...
			// Create the initial event if it does not exist.
			if ( !this.bindedEvents[eventName][refName] )
			{
				var bindedObject = this.bindedEvents[eventName][refName] = this.fireEvent.bind( this, eventName, data );
				this.element.addEventListener( eventName, bindedObject, false );
			}
			this.addAObserver( eventName, refName, method, scope );
		},
		...
		removeNObserver: function ( eventName, functionName, scope )
		{
			...
			console.log("observer is being removed ", eventName, functionName, this.bindedEvents, this.getNumObservers(eventName) );
			// Check bind length, before removing event.
			if ( this.getNumObservers( eventName ) <= 1 )
			{
				this.element.removeEventListener( eventName, this.bindedEvents[eventName][functionName], false );
				delete this.bindedEvents[eventName][functionName];
			}
		},
		...
		{
			//console.log("firing event", events);
	 ...
	 * @returns {Node}
	 */
	$scope.node = function ( nodeName, nodeProperties )
	{
	 ...
	 * @returns {Text}
	 */
	$scope.text = function ( string, isHTML )
	{
	 ...
	 * @returns {Node|Node[]}
	 */
	$scope.P = function ( nodeName, parent )
	{
