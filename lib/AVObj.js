...
{
	var currentScope = window;		// This will store the last element we search into which will represent the namespace.
	...
	//	console.log(spaces[i], currentScope);
	}

	if ( scopeFunction != null )
	{
		scopeFunction.apply( currentScope );
	}
...
function usingNamespace()
{
	...
	for( var i = 0; i < arguments.length - 1; i++ )
	{
		...
		for( var j = 1; j < splitNamespace.length; j++ )
		{
			finalObject = finalObject[splitNamespace[j]];
		}

		var namespaceRef = splitNamespace[splitNamespace.length - 1];

		for( var object in finalObject )
		{
			if ( typeof finalObject[object] === "object" || typeof finalObject[object] === "function")
			{
				namespaceRefList.push( finalObject );
				functionArg.push(finalObject[object]);
				functionArgNames.push(object);
				returnAguments.push( finalObject );
			}
		}
	}
	var functionString = arguments[arguments.length - 1].toString();
	var regExpTest = /^function\s*\(([\s\w\d,]*)\s*\)\s*\{([\S\s]*)}$/g;

	var regResult = regExpTest.exec( functionString );

	//var arguments = regResult[1].split(",");
	var functionBody =  regResult[2];

	functionBody += "return [" + functionArgNames.toString() + "];";

	//for( var i = 0; i < functionArg.watch)

	var changes = Function( functionArgNames, functionBody ).apply( window, functionArg );

	for( var i = 0; i < changes.length; i++ )
	{
		returnAguments[i][functionArgNames[i]] = changes[i];
	}
}
