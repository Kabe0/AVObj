...
function namespace( spaces, scopeFunction )
{
	...
	{
		var regResult = (/^function\s*\([\s\w\d$,]*\s*\)\s*\{([\S\s]*)}$/g).exec( scopeFunction.toString() );
		var scopeChanges = Function( ["$scope"], regResult[1] ).apply( this, [curScope] );
	}
...
function using( spaces )
{
	...
	returnOb.on = function( functionScope )
	{
		// Only bother doing all this work if actual namespaces were defined.
		if ( argLength > 0 )
		{
			var functionString = functionScope.toString();
		...
		{
			arguments[argLength].apply( this );
		}
		return this;
	};

	returnOb.within = function( spaces, functionScope )
	{
		var curScope = window; // This will store the last element we search into which will represent the namespace.
		...
		{
			// Adding the $scope attribute if not already there.
			scopeNames.push( "$scope" );
			scopeArguments.push( curScope );
			...
			// Remove the scope object as we don't want to change that value.
			for ( i = scopeChanges.length - 1; i--; )
			{
				scope[i][scopeNames[i]] = scopeChanges[i];
			}
		}
...
/** @namespace */
var AV = { /** @namespace */ Utilities : {}, /** @namespace */ App : { Display : {} } };				// Defines the main namespace for all AVObject utilities (not necessary, but helps with IDE Editor's.
...
(function ()
{
	...
	var createObject =
	{
		...
		value:        function ()
		{
			...
			// PreInit gives the user a chance to define and configure the object before macro init's and base init fires.
			newObj.preInit.apply( newObj );

			// calls init on any predefined macro objects before calling objects user defined init method.
			for( var i = 0, length = newObj._macroInit.length; i < length; i++ )
			{
				var macroObjName = newObj._macroInit[i];
				//newObj._macroInit[i]();
				macroList[macroObjName].init.call( newObj, newObj._usedMacros[macroObjName], arguments );
			}
	...
	function createFinal ( objectPrototype )
	{
		...
		var newObjectValues =
		{
			...
			super:           {
				...
				value: function ()
				{
					var parentObj = this["_parent" + ( this._extensionCount - this._superTrack++ )];

					parentObj.preInit.apply( this );
					var returnValue = parentObj.init.apply( this, arguments );
					this._superTrack = 0;
 ...
 *     </ol>
	 ...
	 * @param {function=} operationInit - The function to call when the object is constructed (the word create() is called on the object right before the objects own init method is called).
	 * @function  AVObjMacro
	 */
	AVObjMacro = function( macroName, operationBefore, operationAfter, operationInit )
	{
		if ( macroList[macroName] == null )
		{
			macroList[macroName] = { before:operationBefore, after:operationAfter, init:operationInit };
		}
		else
		{
			...
			properties.instance =
			{
				get: function ()
				{
					if ( this._instance == null )
					{
						...
						this.preInit.apply( this._instance );
	...
	AVObj =
	{
		...
		extend: function ( macros, interfaces )
		{
			...
			if ( macros )
			{
				...
				{
					if ( macros[macroName] != null )
					{
						if ( macroList[macroName].before != null )
						{
			...
			if ( macros )
			{
				for ( macroName in macroList )
				{
					if ( macros[macroName] != null )
					{
						if ( macroList[macroName].after != null )
						{
							macroList[macroName].after( newObject, macros[macroName] );
						}

						// We add the init methods to the _macroInit array object here for convenience.
						if ( macroList[macroName].init != null )
						{
							newObject._macroInit.push( macroName );
						}
					}
				}
			}
		...
		addObserver : function( eventName, objectListener )
		{
			var observerArray = this._observers[eventName];

			// observer event has never been created yet...
			if ( !observerArray )
			{
				observerArray = [];
			}

			// Does not contain an observer...
			if ( observerArray.indexOf( objectListener ) == -1 )
			{
				observerArray.push( objectListener );
			}
		},

		removeObserver : function( eventName, objectListener )
		{
			var index = this._observers[eventName].indexOf( objectListener );

			if ( index > -1 )
			{
				this._observers[eventName].splice( index, 1 );
			}
		},
		...
		{
			this._observers = {};	// Resetting the whole observer object.
		},

		fireObservers : function( eventName, methodName, arg )
		{
			// Storing values for performance optimizations.
			var observerArray = this._observers[eventName];

			// Cant' fire a non existent event.
			if ( observerArray )
			{
				for( var i = observerArray.length; i--; )
				{
					var method;		// Another method being stored for performance.
					if ( method = observerArray[methodName] ) method.apply( method, arg  );
				}
			}
		}
	...
	window.addEventListener( "load", function ()
	{
		AVObj._AVLoadList.forEach( function ( object )
		{
			...
			{
				object._instance = createFinal( object );
				object._instance.preInit();
